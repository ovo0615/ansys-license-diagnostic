...
Assert-True 'MPI 密碼不放進命令列或工具記憶體' ($mpiCredential -notmatch '(?i)-password' -and $mpiCredential -notmatch 'Get-Credential|SecureStringToBSTR|plainPassword')
Assert-True 'MPI 註冊使用原生可見互動視窗' ($mpiCredential -match 'Start-IntelMpiCredentialPrompt' -and $mpiCredential -match 'UseShellExecute\s*=\s*\$true' -and $mpiCredential -match "WindowStyle\s*=\s*'Normal'")
Assert-True 'MPI 原生視窗只執行 register 且不帶密碼' ($mpiCredential -match '" -register"' -and $mpiCredential -notmatch 'InputLines')
...
Assert-True 'GUI 有免重新註冊的雙機快速測試' ($source -match '雙機快速測試' -and $source -match 'Start-MpiVerification')
Assert-True '快速測試涵蓋兩節點 hostname' ($mpiCredential -match 'VerifyOnly' -and $mpiCredential -match '-n 2 -ppn 1 -hosts' -and $mpiCredential -match 'hostname\.exe')
Assert-True '快速測試涵蓋 RSM 與 Hydra 連接埠' ($mpiCredential -match '32958' -and $mpiCredential -match 'I_MPI_HYDRA_SERVICE_PORT')
Assert-True '快速測試會驗證 RSM MPI 執行目錄' ($mpiCredential -match 'ANSYS_EM_EXEC_DIR' -and $mpiCredential -match '請先按.*完整修復')
Assert-True '快速測試會提醒關閉既有 AEDT 求解程序' ($mpiCredential -match 'ansysedt' -and $mpiCredential -match 'HFSSCOMENGINE' -and $mpiCredential -match '正式求解前')
Assert-True '快速測試把既有 AEDT 程序視為未通過' ($mpiCredential -match 'verificationBlocked' -and $mpiCredential -match 'exit 3')
...
Assert-True '同事版說明要求允許遠端機器存取' ($guide -match 'Disable access by remote machines' -and $guide -match '不要勾選')
