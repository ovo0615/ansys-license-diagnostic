...
### 2. `New-AedtClusterConfig.ps1`（公開，唯讀輸出）

吃上一步的彙整 JSON，產生：

- `machines.txt` — `-MachineList file=` 用的機器清單
- `cluster.acf` — 匯入 AEDT GUI 用
- `run-batch.bat` — 一行可跑的 `-BatchSolve -Distributed -MachineList ...`
- 一份「還缺什麼」的清單

不碰任何機器，只產檔案。客戶可以先看過再決定要不要用。

### 3. `Initialize-AedtCluster.ps1`（私有，會修改機器）
...
| --- | --- |
| 檢查現況、找出缺什麼 | **可以全自動** |
| 產生機器清單 / acf / 批次檔 | **可以全自動** |
| 首次佈署（裝 MPI、註冊 RSM、開防火牆） | 半自動，每台要管理員跑一次、要人輸密碼 |
...
`Test-AedtCluster.ps1` 的**彙整模式**有 40 項自動測試（`tests\Test-AedtClusterMerge.ps1`），
因為它吃的是 JSON、不碰機器，可以完全用合成資料驗。
**節點收集那一半沒有任何自動測試**——偵測邏輯必須在真的 Windows 工作站上跑過才算數。
第一次到客戶端時要逐項核對報告內容是否與實際狀況相符。

動工做第 2、3 支工具前必須確認：

1. `-MachineList` 各種寫法在**目標版本**的實際語法（用 `ansysedt -Batchoptions` 對照）
2. `.acf` 的實際欄位結構——是否適合程式產生，還是只能由 GUI 匯出後改
3. 近版 AEDT 是否已預設帶 Intel MPI，`hydra_service` 是否隨安裝一起裝
4. 2023 R1 之後 RSM 的角色是否有變（舊教學多半以 IBM Platform MPI 為前提）
5. MPI 實際使用的埠範圍，才能寫出精確的防火牆規則

第 1、2 項沒確認前不要寫產生器；語法猜錯產出的批次檔會直接失敗，比沒有還糟。
