...
12. HPC Pack 是**倍增**而非線性。第 n 個 pack 額外開放 `2 × 4^n` 核，
    加上授權內含的 4 核，單台總核心數為 `2 × 4^n + 4`（n≥1），n=0 時為 4：

    | pack 數 | pack 額外開放 | 單台總核心數 |
    | --- | --- | --- |
    | 0 | 0 | 4 |
    | 1 | 8 | **12** |
    | 2 | 32 | **36** |
    ...
    > 原廠文件與代理商簡報講第 4 個 pack 時常寫「512 核」，那是**只算 pack 開放的部分**，
    > 省略了授權內含的 4 核。本工具一律報總核心數，所以是 516。
    > 兩個數字都對，差別在算不算內含的 4 核——跟客戶對數字時要先講清楚是哪一種。
